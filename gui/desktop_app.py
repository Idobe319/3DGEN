...
class LiveGraph(ctk.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        self.data = [30] * 50
        self.running = True
        self._external_value: int | None = None
        self.animate()

    def set_value(self, v: int | None) -> None:
        """Allow external components to set the displayed load value (0-100)."""
        try:
            if v is None:
                self._external_value = None
            else:
                self._external_value = int(max(0, min(100, v)))
        except Exception:
            self._external_value = None
    ...
    def animate(self):
        ...
        # Use external GPU value if present, otherwise generate sensible random noise
        last = self.data[-1]
        if self._external_value is not None:
            # Smooth toward target
            target = max(10, min(90, int(self._external_value / 1.0)))
            delta = target - last
            step = int(max(-5, min(5, delta)))
            new_val = last + step + random.randint(-2, 2)
            new_val = max(5, min(95, new_val))
        else:
            new_val = last + random.randint(-5, 5)
        ...
        w = self.winfo_width()
        h = 60
        step = w / len(self.data) if len(self.data) > 0 else w

        points = []
        for i, val in enumerate(self.data):
            x = i * step
            y = h - (val / 100.0) * h if self._external_value is not None else h - val
            points.append(x)
            points.append(y)
...
class NeoForgeUltimate(ctk.CTk):
    ...
    def _build_telemetry(self):
        ...
        # Start periodic GPU polling for improved telemetry (uses neoforge_core._gpu_stats)
        try:
    ...
    def _start_gpu_poll(self):
        """Periodically poll for system-level GPU stats via `neoforge_core._gpu_stats` and update UI."""
        try:
            from neoforge_core import _gpu_stats
        except Exception:
            return
        ...
        def poll():
            try:
                stats = _gpu_stats()
                if stats:
                    # Prefer bytes-based memory values
                    mem_used = stats.get('memory_used') or stats.get('memory_allocated')
                    mem_total = stats.get('memory_total') or stats.get('memory_reserved')
                    util = stats.get('utilization')
                    dev = stats.get('device') or stats.get('name') or 'GPU'
                    if mem_used is not None and mem_total is not None:
                        self.lbl_gpu.configure(text=f"{dev} {int(mem_used/(1024*1024))}MB/{int(mem_total/(1024*1024))}MB ({int(util) if util is not None else 0}%)")
                        if isinstance(self.graph, LiveGraph):
                            perc = int(util if util is not None else (mem_used / max(1, mem_total) * 100))
                            self.graph.set_value(perc)
                    elif 'memory_allocated' in stats:
                        self.lbl_gpu.configure(text=f"{dev} {int(stats.get('memory_allocated',0)/(1024*1024))}MB alloc")
            except Exception:
                pass
            # schedule next poll
            try:
    ...
    def _handle_progress(self, payload: dict) -> None:
        ...
        if isinstance(details, dict):
            ...
            # GPU info inside payload (prefer explicit GPU telemetry)
            gpu = details.get('gpu')
            if isinstance(gpu, dict):
                mem_used = gpu.get('memory_used') or gpu.get('memory_allocated')
                mem_total = gpu.get('memory_total') or gpu.get('memory_reserved')
                util = gpu.get('utilization')
                name = gpu.get('device') or gpu.get('name')
                if mem_used is not None and mem_total is not None:
                    self.lbl_gpu.configure(text=f"{name} {int(mem_used/(1024*1024))}MB/{int(mem_total/(1024*1024))}MB ({int(util) if util is not None else 0}%)")
                    # push to graph as percentage
                    if isinstance(self.graph, LiveGraph):
                        u = int(util if util is not None else (mem_used / max(1, mem_total) * 100))
                        self.graph.set_value(u)
                else:
